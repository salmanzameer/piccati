...
def create
 # binding.pry
  ...
  if @client.save
   redirect_to   api_v1_photographer_client_path(current_photographer,@client.id)
 else
  flash[:notice] ="Error"
  render('new')
 end
...
def update
  ...
  if @client.save
   ...
   redirect_to api_v1_photographer_clients_path(current_photographer)
  else
    render ('edit')
 end 
end
...
# def upload_images
     ...
     #   current_photographer.clients.find_by_id(params[:id]).images.create(param)

     #    #redirect_to upload_images_photographer_client_path(current_photographer,@client.id)
     # end
     # def method_name
